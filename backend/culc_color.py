...
colors_checker_arr_origin = [[115, 82, 68], [194, 150, 130], [98, 122, 157], [87, 108, 67], [133, 128, 177], [103, 189, 170],
...
def detect_squares_checker(img):
    ...
    for i in range(4):
        for j in range(6):
            ROI = img[i* int(h/4) + offset_h : i*int(h/4) + offset_h + square_h, j * int(w/6) + offset_w : j*int(w/6) + offset_w + square_w]
...
def calc_func(colors_checker_arr_origin, colors_checker_arr):
...
def find_RGB(p_list, photo_color):
    p_r, p_g, p_b = p_list
...
def get_RGB(board_path, color_test):
    global colors_checker_arr
    img = cv2.imread(board_path, cv2.IMREAD_COLOR)
    color_test = cv2.imread(color_test, cv2.IMREAD_COLOR)
    ...
    print(len(colors_checker_arr))
    print(len(colors_checker_arr_origin))
    f = calc_func(colors_checker_arr_origin, colors_checker_arr)

    b, g, r = find_RGB(f, photo_med_color)
    colors_checker_arr = []
    return r, g, b
...
def get_n_closest(rgb: (int, int, int), n: int) -> List[dict]:
    print(os.path.join('.\\backend\\fan_colors.csv'))
    fan_colors_df = pd.read_csv('.\\backend\\fan_colors.csv')
